...
class Model:
    ...
    def train_new(self, channel):
        """
        Train TCN model according to specifications in config.yaml.
        ...
        self.model.add(TCN(nb_filters=64, kernel_size=2,dilations=[1, 2, 4, 8, 16, 32],return_sequences=True))
        self.model.add(Dropout(self.config.dropout))

        self.model.add(TCN(nb_filters=64, kernel_size=2,dilations=[1, 2, 4, 8, 16, 32],return_sequences=False))
        self.model.add(Dropout(self.config.dropout))
    ...
    def batch_predict(self, channel):
        """
        Used trained TCN model to predict test data arriving in batches.
